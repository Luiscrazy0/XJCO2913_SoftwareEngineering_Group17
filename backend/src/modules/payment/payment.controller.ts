...
export class PaymentController {
  ...
  @ApiOperation({ summary: '创建支付', description: '为预约创建支付记录（需要登录）' })
...
*/
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: '获取支付信息', description: '根据预约ID获取支付信息（需要登录）' })
  @ApiParam({ name: 'bookingId', description: '预约ID', example: 'clx0987654321' })
  @ApiResponse({ 
    status: 200, 
    description: '获取成功',
    schema: {
