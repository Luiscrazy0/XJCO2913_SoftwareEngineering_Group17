...
export class StatisticsController {
  ...
  async getWeeklyRevenueByHireType(
    ...
    const data = await this.statisticsService.getWeeklyRevenueByHireType(startDate, endDate);
    ...
    const dtoData: WeeklyRevenueByHireTypeDto[] = data.map(item => ({
      ...
      hireTypeName: this.statisticsService.getHireTypeChineseName(item.hireType),
  ...
  async getDailyRevenue(
    ...
    const data = await this.statisticsService.getDailyRevenue(startDate, endDate);
    ...
    const dtoData: DailyRevenueDto[] = data.map(item => ({
      ...
      hireTypes: item.hireTypes.map(hireType => ({
        ...
        hireTypeName: this.statisticsService.getHireTypeChineseName(hireType.hireType),
  ...
  @ApiQuery({ name: 'period', required: false, description: '统计周期 (week, month, year)', enum: ['week', 'month', 'year'] })
  @ApiQuery({ name: 'type', required: false, description: '图表类型 (bar, line, pie)', enum: ['bar', 'line', 'pie'] })
  @ApiResponse({ 
    status: 200, 
  ...
  async getRevenueChartData(
    ...
    const chartData = await this.statisticsService.getRevenueChartData(period, type);
