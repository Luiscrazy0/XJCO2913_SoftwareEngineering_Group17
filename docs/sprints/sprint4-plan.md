...
### 12.5 补充完成项（超出 Sprint 4 范围）

| 项目 | 说明 |
|------|------|
| **Backlog ID 16 — 价格配置** | `AdminPricingPage.tsx` + `priceApi.getPricing/updatePricing/resetPricing`。管理员可通过 `/admin/pricing` 页面实时调整各租赁类型单价，支持内联编辑（Enter/Escape 快捷键）、价格对比表、重置默认值。后端 API 在 Sprint 4 已实现，前端管理 UI 补充完成 |
| **前端商业化优化** | Glass-morphism 设计系统、页面过渡动画（fade-in-up/stagger）、移动端底部导航栏、刘海屏安全区域支持、深色主题 Toast 系统、Navbar 毛玻璃粘性定位、PageLayout 统一页面布局、Footer |
| **开屏广告** | `SplashScreen.tsx` — 全屏推广弹窗，3条轮播广告（夏日骑行/月卡/学生优惠），localStorage 冷却机制（30分钟） |
| **骑行套餐页** | `/ride-packages` — 4档套餐（单次/10次卡 ¥129/月卡 ¥199/季卡 ¥499），套餐对比表，FAQ 折叠面板 |
| **侧边栏广告** | `SidePromo.tsx` — 桌面端侧边推广卡片（月卡升级 + 邀请好友），集成到车辆列表页和骑行套餐页 |
