...
export default function FeedbackDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-main)]">
        ...
        <div className="max-w-6xl mx-auto px-4 py-16 text-center">
          ...
          <p className="mt-4 text-[var(--text-secondary)]">Loading feedback details…</p>
        </div>
      </div>
    )
  }
  ...
  // Error
  if (isError || !feedback) {
    const message = error instanceof Error ? error.message : 'Feedback not found'
    return (
      <div className="min-h-screen bg-[var(--bg-main)]">
        <Navbar />
    ...
    <div className="min-h-screen bg-[var(--bg-main)]">
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-6 py-10 space-y-8">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-[var(--text-secondary)]">Admin · Feedback Detail</p>
            <h1 className="text-3xl font-bold text-[var(--text-main)]">Feedback Details</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
              <h2 className="text-xl font-semibold text-[var(--text-main)] mb-4">Feedback Information</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Title</p>
                  <p className="text-lg font-medium text-[var(--text-main)]">{feedback.title}</p>
                </div>
                
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Category</p>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Priority</p>
                  {isEditing ? (
                    <select
                      name="priority"
                ...
                <div className="md:col-span-2">
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Description</p>
                  <p className="text-[var(--text-main)] mt-1 whitespace-pre-wrap">{feedback.description}</p>
                </div>

                {feedback.imageUrl && (
                  <div className="md:col-span-2">
                    <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Image</p>
                    <div className="mt-2">
                      <img 
            ...
            {/* Manager Notes Card */}
            <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
              <h2 className="text-xl font-semibold text-[var(--text-main)] mb-4">Manager Notes</h2>
              {isEditing ? (
                <textarea
                  name="managerNotes"
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
              ...
              <div className="space-y-4">
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => handleDamageTypeChange('INTENTIONAL')}
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${feedback.damageType === 'INTENTIONAL' ? 'bg-red-500 text-white' : feedback.damageType === 'NATURAL' ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'}`}>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)] mb-2">Resolution Cost</p>
                  {isEditing ? (
                ...
                {feedback.status === 'CHARGEABLE' && feedback.resolutionCost && feedback.resolutionCost > 0 && (
                  <div className="rounded-lg bg-red-500/10 border border-red-500/30 p-4">
                    <p className="text-sm font-medium text-red-200">Chargeable Damage</p>
                    <p className="text-xs text-red-200/80 mt-1">
                      This feedback requires payment of ${feedback.resolutionCost.toFixed(2)} from the user.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
              <h2 className="text-xl font-semibold text-[var(--text-main)] mb-4">Related Information</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">Location: {feedback.scooterLocation}</p>
                </div>

                {feedback.bookingId && (
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Booking</p>
                    <p className="text-[var(--text-main)] font-medium">ID: {feedback.bookingId}</p>
                    {feedback.bookingStartTime && (
                      <p className="text-sm text-[var(--text-secondary)]">
                        Start: {new Date(feedback.bookingStartTime).toLocaleString()}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Last Updated</p>
            ...
            {/* Timeline Card */}
            <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
              <h2 className="text-xl font-semibold text-[var(--text-main)] mb-4">Timeline</h2>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[var(--text-main)]">Feedback Created</p>
                    <p className="text-xs text-[var(--text-secondary)]">
                      {new Date(feedback.createdAt).toLocaleString()}
                    </p>
                  </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[var(--text-main)]">Last Updated</p>
                    <p className="text-xs text-[var(--text-secondary)]">
                      {new Date(feedback.updatedAt).toLocaleString()}
                    </p>
                  </div>
                ...
                {feedback.status === 'RESOLVED' && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[var(--text-main)]">Resolved</p>
                      <p className="text-xs text-[var(--text-secondary)]">
                ...
                {feedback.status === 'CHARGEABLE' && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[var(--text-main)]">Chargeable</p>
                      <p className="text-xs text-[var(--text-secondary)]">
                        Requires payment: ${feedback.resolutionCost?.toFixed(2) || '0.00'}
                      </p>
                    </div>
                  </div>
                )}
