...
export default function UserManagementPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () =>
      usersApi.getUsers({ page, limit, search: searchQuery || undefined }),
  });

  const updateMutation = useMutation({
    mutationFn: ({ userId, userType }: { userId: string; userType: string }) =>
      usersApi.updateUserType(userId, userType),
  ...
  const handleUpdateUserType = useCallback(
    (userId: string, newType: UserTypeOption) => {
  ...
  if (isLoading && !data) {
    return (
      <PageLayout title="用户管理" subtitle="加载中...">
        <LoadingSpinner size="large" showText text="正在加载用户数据…" />
      </PageLayout>
    );
  }
        ...
        <div className="mt-4 flex items-center justify-between border-t border-[var(--border-line)] pt-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page <= 1}
              className="rounded-lg border border-[var(--border-line)] px-3 py-1.5 text-sm text-[var(--text-main)] hover:bg-white/5 disabled:opacity-40"
            >
              上一页
            </button>
            <button
