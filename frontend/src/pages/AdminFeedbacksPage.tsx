...
export default function AdminFeedbacksPage() {
  ...
  if (isLoading) {
    return (
      <PageLayout
        title="反馈管理"
        subtitle="查看、筛选和管理所有用户反馈报告。"
      >
        <LoadingSpinner size="large" showText text="正在加载反馈数据…" />
      </PageLayout>
    );
  }
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border-line)]">
                    <th className="py-3 px-4 text-left text-sm font-semibold text-[var(--text-secondary)]">
                      工单ID
                    </th>
                    <th className="py-3 px-4 text-left text-sm font-semibold text-[var(--text-secondary)]">
                      标题
                ...
                <tbody>
                  {feedbacks.map((feedback) => (
                    <tr
                      key={feedback.id}
                      className="border-b border-[var(--border-line)] hover:bg-white/5"
                    >
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(feedback.category)}`}
                        >
                          {feedback.category}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        ...
                        <div className="text-xs text-[var(--text-secondary)]">
                          电动车位置: {feedback.scooterLocation}
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <div className="text-sm text-[var(--text-main)]">
