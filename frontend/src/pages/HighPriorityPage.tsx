...
export default function HighPriorityPage() {
  ...
  if (!user) {
    return (
      <PageLayout title="高优先级问题" subtitle="需要登录" showBottomNav={false}>
        <div className="rounded-2xl border border-yellow-500/40 bg-yellow-500/15 p-6 shadow-sm text-center">
          ...
          <div className="mt-4">
            <a href="/auth" className="px-4 py-2 rounded-lg bg-[var(--mclaren-orange)] text-white font-semibold hover:brightness-110 inline-block">前往登录</a>
          </div>
        </div>
      </PageLayout>
    )
  ...
  if (user.role !== 'MANAGER') {
    return (
      <PageLayout title="高优先级问题" subtitle="权限不足" showBottomNav={false}>
        <div className="rounded-2xl border border-red-500/40 bg-red-500/15 p-6 shadow-sm text-center">
          ...
          <div className="mt-4">
            <a href="/" className="px-4 py-2 rounded-lg bg-[var(--mclaren-orange)] text-white font-semibold hover:brightness-110 inline-block">返回首页</a>
          </div>
        </div>
      </PageLayout>
    )
  ...
  if (isLoading) {
    return (
      <PageLayout title="高优先级问题仪表板" subtitle="监控和管理需要立即关注的紧急反馈" showBottomNav={false}>
        <div className="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-[var(--mclaren-orange)] border-t-transparent" />
        <p className="mt-4 text-[var(--text-secondary)] text-center">正在加载高优先级问题…</p>
      </PageLayout>
    )
  }
  ...
  if (isError) {
    ...
    if (message.includes('400')) {
      detailedMessage = '请求参数错误或缺少必要信息。请确保后端服务正在运行'
    ...
    return (
      <PageLayout title="高优先级问题仪表板" subtitle="数据加载失败" showBottomNav={false}>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex gap-3">
            <Link to="/admin/feedbacks" className="rounded-lg border border-[var(--border-line)] px-4 py-2 text-sm font-semibold text-[var(--text-main)] hover:border-[var(--mclaren-orange)] hover:bg-white/5 shadow-sm">所有反馈</Link>
            <button onClick={() => refetch()} className="rounded-lg border border-[var(--border-line)] px-4 py-2 text-sm font-semibold text-[var(--text-main)] hover:border-[var(--mclaren-orange)] hover:bg-white/5 shadow-sm">刷新</button>
          </div>
        </header>
        ...
        <div className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-5 shadow-[var(--shadow-card)]">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              {([
                ['ALL', '全部', feedbacks.length],
                ['DAMAGE', '损坏', damageCount],
                ['FAULT', '故障', feedbacks.filter(f => f.category === 'FAULT').length],
                ['SUGGESTION', '建议', feedbacks.filter(f => f.category === 'SUGGESTION').length],
              ] as const).map(([key, label, count]) => {
                const colorMap: Record<string, string> = {
                  ALL: 'bg-[var(--mclaren-orange)] text-white',
                  DAMAGE: 'bg-red-600 text-white',
                  FAULT: 'bg-orange-600 text-white',
                  SUGGESTION: 'bg-blue-600 text-white',
                }
                return (
                  <button
                    key={key}
                    onClick={() => setFilterCategory(key)}
                    className={`rounded-lg px-4 py-2 text-sm font-semibold ${filterCategory === key ? colorMap[key] : 'border border-[var(--border-line)] text-[var(--text-main)] hover:border-[var(--mclaren-orange)] hover:bg-white/5'}`}
                  >
                    {label} ({count})
                  </button>
                )
        ...
        {sortedFeedbacks.length > 0 ? (
          <div className="space-y-4">
            {sortedFeedbacks.map((feedback) => (
              <div key={feedback.id} className="rounded-2xl border border-[var(--border-line)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)] hover:shadow-lg transition-shadow">
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getCategoryColor(feedback.category)}`}>{feedback.category}</span>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getPriorityColor(feedback.priority)}`}>{feedback.priority}</span>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(feedback.status)}`}>{feedback.status}</span>
                      {feedback.damageType && (
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${feedback.damageType === 'INTENTIONAL' ? 'bg-red-500 text-white' : 'bg-green-500 text-white'}`}>{feedback.damageType}</span>
                      )}
                    </div>
          ...
          <div className="rounded-2xl border border-dashed border-[var(--border-line)] bg-[var(--bg-card)] p-10 text-center shadow-[var(--shadow-card)]">
            ...
            <div className="mt-4">
              <button onClick={() => setFilterCategory('ALL')} className="rounded-lg bg-[var(--mclaren-orange)] px-5 py-2 text-sm font-semibold text-white shadow hover:brightness-110">查看所有问题</button>
            </div>
          </div>
        )}
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => {
                    ...
                    const csvData = sortedFeedbacks.map(f => [f.id, f.title, f.category, f.priority, f.status, f.createdByEmail, f.scooterId, new Date(f.createdAt).toISOString(), f.resolutionCost || '0'])
                ...
                <button onClick={() => alert('PDF export functionality would be implemented with a proper PDF generation library.')} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow hover:brightness-110">导出PDF</button>
              </div>
            </div>
          </div>
        )}
