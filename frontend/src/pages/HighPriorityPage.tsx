...
export default function HighPriorityPage() {
  ...
  if (isLoading) {
    return (
      <PageLayout
        title="高优先级问题仪表板"
        subtitle="监控和管理需要立即关注的紧急反馈"
        showBottomNav={false}
      >
        <LoadingSpinner size="large" showText text="正在加载高优先级问题…" />
      </PageLayout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-8">
              ...
              {(
                [
                  ...
                  [
                    "FAULT",
                    "故障",
                    feedbacks.filter((f) => f.category === "FAULT").length,
                  ],
                  [
                    "SUGGESTION",
                    "建议",
        ...
        {sortedFeedbacks.length > 0 ? (
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      ...
                      {feedback.damageType && (
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${feedback.damageType === "INTENTIONAL" ? "bg-red-500 text-white" : "bg-green-500 text-white"}`}
                        >
                          {feedback.damageType}
                        </span>
                      )}
                    ...
                    {feedback.resolutionCost && feedback.resolutionCost > 0 && (
                      <div className="mt-4 p-3 rounded-lg bg-red-500/10 border border-red-500/30">
                        ...
                        <p className="text-lg font-bold text-red-200">
                          ${feedback.resolutionCost.toFixed(2)}
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="rounded-2xl border border-dashed border-[var(--border-line)] bg-[var(--bg-card)] p-10 text-center shadow-[var(--shadow-card)]">
            ...
            <h3 className="mt-4 text-lg font-semibold text-[var(--text-main)]">
              无高优先级问题
            </h3>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() =>
                    alert(
                      "PDF export functionality would be implemented with a proper PDF generation library.",
                    )
                  }
