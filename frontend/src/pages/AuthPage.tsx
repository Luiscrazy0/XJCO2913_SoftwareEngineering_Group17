...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg-main)] px-4 py-8 relative" role="main" id="main-content">
      ...
      <div className="glass-card w-full max-w-md p-8 md:p-10 relative animate-fade-in-up">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <svg className="w-12 h-12 text-[var(--mclaren-orange)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-[var(--text-main)] mb-2">电动滑板车租赁系统</h1>
          <p className="text-sm text-[var(--text-secondary)]">欢迎使用我们的租赁服务</p>
        </div>

        {/* Tabs */}
        <div className="flex border-b border-[var(--border-line)] mb-6" role="tablist">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="email" className="text-sm font-medium text-[var(--text-main)]">
              邮箱地址 <span className="text-[var(--text-secondary)]">（必填）</span>
            </label>
            <input
              id="email"
              type="email"
            ...
            />
            {formErrors.email && (
              <div id="email-error" className="text-xs text-rose-400" role="alert">{formErrors.email}</div>
            )}
          </div>

          {/* Password */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="password" className="text-sm font-medium text-[var(--text-main)]">
              密码 <span className="text-[var(--text-secondary)]">（必填）</span>
            </label>
            <input
              id="password"
              type="password"
            ...
            />
            {formErrors.password && (
              <div id="password-error" className="text-xs text-rose-400" role="alert">{formErrors.password}</div>
            )}
          </div>
          ...
          {activeTab === 'register' && (
            <div className="flex flex-col gap-1.5">
              <label htmlFor="confirmPassword" className="text-sm font-medium text-[var(--text-main)]">
                确认密码 <span className="text-[var(--text-secondary)]">（必填）</span>
              </label>
              <input
                id="confirmPassword"
                type="password"
              ...
              {formErrors.confirmPassword && (
                <div id="confirm-password-error" className="text-xs text-rose-400" role="alert">{formErrors.confirmPassword}</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'register' && (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-start gap-3">
                ...
                />
                <label htmlFor="insuranceAcknowledged" className="text-sm font-medium text-[var(--text-main)] cursor-pointer">
                  我已阅读并同意保险条款 <span className="text-[var(--text-secondary)]">（必填）</span>
                </label>
              </div>
          ...
          {activeTab === 'register' && (
            <div className="flex flex-col gap-1.5">
              <label htmlFor="emergencyContact" className="text-sm font-medium text-[var(--text-main)]">
                紧急联系人 <span className="text-[var(--text-secondary)]">（可选）</span>
              </label>
              <input
                id="emergencyContact"
                type="text"
              ...
              {formErrors.emergencyContact && (
                <div id="emergency-contact-error" className="text-xs text-rose-400" role="alert">{formErrors.emergencyContact}</div>
              )}
          ...
          {/* Error Message */}
          {error && (
            <div className="p-3 border border-rose-500/30 bg-rose-500/10 rounded-lg text-sm text-rose-200 text-center" role="alert" aria-live="assertive">
              {error}
            </div>
          )}
